...
<div class="container">
  ...
  <h1>CeDARC Number Information</h1>
  <a href="index.php" class="btn btn-dark mb-3">Back</a>
  <a href="cedarc_exnew.php" class="btn btn-dark mb-3">Add New</a>
  <table id="pathology_lab">
  <tr class="header">
  <table class="table table-hover text-center">
  <thead class="table-dark">
    <tr>
      <th scope="col">Cedarc Number</th>
  ...
  <tbody>
    ...
    $sql="SELECT * FROM cedarc_external ";  
    if (!empty($search)) {
      $sql .= " WHERE cedarc_number LIKE '%$search%' OR patient_name LIKE '%$search%'";
  }                              
    $result = mysqli_query($conn, $sql);  
    if ($result && mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
        echo '<tr>
           <td>' . $row['cedarc_number'] . '</td>
           ...
           <td>' . $row['report_deliver_date'] . '</td>
            ...
            <a href="cedarc_external.php" class="link-dark" onclick="confirmDelete("cedarc_number")">
